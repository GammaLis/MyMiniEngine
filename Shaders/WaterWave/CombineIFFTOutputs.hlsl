...
void main( uint3 dtid : SV_DispatchThreadID )
{
	...
	// displacement
	DisplacementMap[dtid.xy] = float4(Dx, Dh, Dy, 0.0f);

	float xyScale = _L / (_N-1);
	...
	/// normal & fold may have some errors!	-2020-8-3
	// normal
	...
	float jyx = _Lambda * (ry - ly) / (2.0f * xyScale);
	float j = jxx * jyy - jxy * jyx;
	float fold = max(0.0f, 1.0f - saturate(j));

	NormalAndFoldMap[dtid.xy] = float4(normal, fold);
